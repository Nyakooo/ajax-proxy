...
- [ ] **依赖安全告警处置**：按分支锁文件分别审计生产 / 开发依赖、严重级别和 fixed version 并分批修复。GitHub push 提示的 133 条 Dependabot 告警属于默认分支 `master`，不代表当前 V3 分支；`refactor/v3` 的官方 registry 完整审计现为 21 条（4 高、15 中、2 低），生产审计为一个低危 Vue 2 公告。各项升级单独验证构建和浏览器兼容性，见 `docs/DEPENDENCY-SECURITY.zh.md`。
