...
- [x] 为 V3 增加 V7 Fetch-only 函数重定向：隔离执行仅接收原始 URL / method 并返回经校验的 HTTP(S) URL；sandbox 故障 fail-open 到原请求、保留组合响应，XHR 不执行函数。函数保存 / 启用分别确认，导入代码保留但停用 request action；V2 格式仍不兼容。
...
- [x] 更新中英文 README、安装、使用、规则配置和 FAQ。
- [x] 说明自定义函数能力与风险，补充可复制的示例。
- [x] 说明 V2 与 V3 配置 / 备份不兼容，并提供 V3 备份、恢复和重新配置指南；详见 `docs/V3-BACKUP-RESTORE.zh.md`。
- [x] 维护版本号策略、变更日志格式和发布检查表；详见 `docs/V3-RELEASE.zh.md`。
- [x] 从干净 clone 安装锁定依赖并执行完整 clean build，确认生成扩展 manifest 与 `panels-v3/index.html`（2026-09-26，Node 24.21.0 / pnpm 12.6.0）。
...
- [ ] **依赖安全告警处置**：按分支锁文件分别审计生产 / 开发依赖、严重级别和 fixed version 并分批修复。GitHub push 提示的 133 条 Dependabot 告警属于默认分支 `master`，不代表当前 V3 分支；`refactor/v3` 的 npm 官方 registry 完整审计现为 2 条（0 高、1 中、1 低），分别是无 Vue 2 修复版本的 `vue-template-compiler` 开发依赖公告和迁移 Vue 3 前保留的低危 Vue 2 生产公告。可修复的工具链公告已分批处理，尚余两条均由 Vue 2 工具链迁移关闭，见 `docs/DEPENDENCY-SECURITY.zh.md`。
  ...
  - [x] **external-editor tmp 路径遍历修复**：将 `external-editor@3.1.0` 的 `tmp` 精确升级至 0.2.7，覆盖原路径遍历及 0.2.6 绕过公告；API smoke、全量本地验证和 CI run [36247139586](https://github.com/Nyakooo/ajax-proxy/actions/runs/36247139586) 的 Stable / 最低版本 Chrome 与 Edge 扩展 smoke 通过。
  - [x] **serialize-javascript 工具链修复**：仅覆盖 copy-webpack-plugin 9 / 11 与 css-minimizer-webpack-plugin 3 的依赖至 7.1.2；三个父包 CommonJS 序列化 smoke、全量本地验证及 CI run [36247634483](https://github.com/Nyakooo/ajax-proxy/actions/runs/36247634483) 的 Stable / 最低版本 Chrome 与 Edge 扩展 smoke 均通过。
  - [x] **Vue 2 编译链 PostCSS 修复**：仅将 `@vue/component-compiler-utils@3.3.0` 的 PostCSS 7 边定向升级至 8.5.28。scoped / trim、SCSS、插件、source-map API smoke 与全量构建和 CI run [36248082897](https://github.com/Nyakooo/ajax-proxy/actions/runs/36248082897) 的 Stable / 最低版本 Chrome 与 Edge 扩展 smoke 均通过；旧插件 API 弃用提示留待编译链迁移处理。
  - [x] **hosted-git-info 同范围修复**：两个 normalize-package-data 父包均在声明范围内升级到 2.8.9；父包 API smoke、全量验证及 CI run [36248444075](https://github.com/Nyakooo/ajax-proxy/actions/runs/36248444075) 的完整浏览器矩阵通过。
...
- [x] 测试请求进行中关闭隐私诊断后，不会迟到派发 Fetch / XHR outcome 事件；覆盖 Fetch 原生请求挂起后及 XHR `send()` 后关闭开关的情况。
...
- [x] 为保留能力补齐回归与文档示例：V3 正则 URL 匹配有单元测试，并由扩展 smoke 验证 Fetch / XHR 对 `/api/items` 命中、对 `/api/nope` 回源；状态码自定义和 Fetch 函数响应沿用现有覆盖。V2 旧函数重定向补成功路径测试和用法示例，并注明未迁移到 V3。
...
- 2026-09-26：完成旧 Vue CLI / ESLint 开发依赖的同范围安全补丁：以父包限定 overrides 升级 Acorn、三条 ansi-regex 主版本线、css-what、nth-check、normalize-url，以及 semver 2 / 5 / 6 的受影响边；没有更改生产依赖。官方完整审计由 33 条（16 高 / 15 中 / 2 低）降至 23 条（6 高 / 15 中 / 2 低），生产低危 Vue 保持不变。全量 build、typecheck、345 项 Vitest、包边界、声明、迁移 lint、格式及生产审计通过；CI run `36246275038` 的 Stable 和最低版本 Chrome / Edge、扩展 Fetch / XHR E2E 全部通过。完成 325 / 360 项（90.3%）。
- 2026-09-26：完成旧 ESLint 缓存链 flatted 安全补丁：`flat-cache@2.0.1` 定向改用 `flatted@3.4.4`，真实运行 ESLint 6 缓存写入 / 读取均成功。官方完整审计由 23 条（6 高 / 15 中 / 2 低）降至 21 条（4 高 / 15 中 / 2 低）；全量 build、345 项 Vitest、typecheck、声明、边界、迁移 lint、格式、生产审计及签名通过。CI run `36246723057` 的 Chrome / Edge Stable、最低版本扩展 smoke 和构建全部成功。完成 326 / 361 项（90.3%）。
- 2026-09-26：完成 `external-editor@3.1.0` 所带 `tmp@0.0.33` 路径遍历修复，限定更新到 `tmp@0.2.7` 以同时覆盖 `0.2.6` 的绕过漏洞。通过 external-editor 解析路径调用 `tmp.tmpNameSync` 的 API smoke；全量 build、345 项 Vitest、typecheck、声明、边界、迁移 lint、格式、审计及 1,360 个 registry 签名验证通过。CI run `36247139586` 的 Stable 与最低版本 Chrome / Edge Fetch / XHR smoke 全部成功。官方完整审计降至 19 条（3 高 / 15 中 / 1 低），仍有未处理高危依赖，依赖安全工作继续。完成 327 / 362 项（90.3%）。
- 2026-09-26：完成 `serialize-javascript@6.0.0` 开发工具链升级：对三个实际父包做精确 override 至 7.1.2；逐一验证 CommonJS 函数导出及 RegExp、Date、options 序列化。全量 build、345 项 Vitest、typecheck（在 build 完成后运行）、声明、边界、迁移 lint、格式、生产审计及 1,359 个 registry 签名验证通过。CI run `36247634483` 的 Chrome / Edge Stable 和 Chrome 141 / Edge 140 最低版本扩展 smoke 全部成功。完整审计降至 16 条（2 高 / 13 中 / 1 低）。完成 328 / 363 项（90.4%）。
- 2026-09-26：完成 Vue 2 编译工具的 PostCSS 安全修复：对 `@vue/component-compiler-utils@3.3.0` 的 PostCSS 7 依赖边精确覆盖至现有 PostCSS 8.5.28。局部 compileStyle smoke 验证 scoped / trim、SCSS、自定义插件和 source map；全量 Vue 2 / Vue 3 / 扩展构建、345 项 Vitest、typecheck、声明、边界、迁移 lint、格式、生产审计及 1,357 个 registry 签名验证通过。CI run `36248082897` 的 Stable / 最低版本 Chrome 与 Edge 扩展 Fetch / XHR smoke 全绿。完整审计降至 11 条（0 高 / 10 中 / 1 低）。旧 postcss.plugin API 打印弃用提示，留作 Vue 2 编译链迁移清理。完成 329 / 364 项（90.4%）。
- 2026-09-26：完成 `hosted-git-info` 两个 legacy 分支的同范围安全补丁：`normalize-package-data@2.3.2` / `2.5.0` 的子依赖均升级至 2.8.9；分别通过父包解析 GitHub URL 的 API smoke。全量 build、345 项 Vitest、typecheck、声明、边界、迁移 lint、格式、生产审计与 1,356 个签名验证通过。CI run `36248444075` 的 Chrome / Edge Stable 与最低版本扩展 smoke 全绿。审计降至 10 条（0 高 / 9 中 / 1 低）。完成 330 / 365 项（90.4%）。
