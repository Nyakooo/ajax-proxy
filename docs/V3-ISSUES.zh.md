...
### Fetch 从 `Request` 读取 method 时规则不匹配

- 状态：已复现，待修复。
- 影响范围：`packages/proxy-lib/src/createFetch.ts` 的拦截规则 method 匹配；重定向 Fetch 也从 `init.method` 读取 method。
...
- 实际结果：返回 body 为 `original`，POST 规则未应用。源码只从 `init.method` 读取 method，因此把 Request 自身的 POST 当作 `ANY`。
- 预期结果：读取 `Request.method`；规则按实际 POST 匹配，并让通知及函数响应上下文使用相同 method。还需覆盖 `init.method` 覆盖 Request.method 以及默认 GET 的边界。
- 建议优先级：高；Fetch / XHR 关键语义及规则匹配进入阶段 2，并补持久化回归测试。
