...
| `packages/v3-domain/src/index.ts` 与 `backup.ts`            | 包根公共 barrel 汇出 backup schema / validator 和 matcher；`backup.ts` 负责 V3 envelope / schema / JSON 校验           | 继续保持 public entry 稳定，避免规则匹配器依赖 barrel；类型与 schema 若继续拆分，必须留在 domain 包内             |
