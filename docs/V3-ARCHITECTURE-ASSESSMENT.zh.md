...
| `packages/v3-domain/src/{index,backup,rules,ruleMatching}.ts` | 包根公共 barrel 汇出 backup schema / validator、规则类型和 matcher；`backup.ts` 负责 envelope / JSON 校验，`rules.ts` 定义纯规则模型             | 继续保持 public entry 稳定；backup validator 与 matcher 都只依赖规则模型，不从包根 barrel 导入内部实现            |
