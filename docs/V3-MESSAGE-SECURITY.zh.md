...
| content script         | 页面主世界命中事件                | 分别验证 V2 与 V3 hit payload；只接受 plain record，检查规则索引 / ID、匹配 URL、HTTP method、可选请求 URL 的类型和长度，拒绝额外字段和异常对象。 |
| service worker         | content script                    | `sender.id` 必须是本扩展 ID，且必须有 tab；只接受受限的标题初始化或命中数据。V3 hit 会再对照有效活动 backup 与 rule id / URL / method。           |
