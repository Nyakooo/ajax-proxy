...
- 使用 npm 官方 registry 执行 `pnpm audit --json`：当前 `refactor/v3` 锁文件有 10 个公告（0 高、9 中、1 低、0 严重）；其中 9 个属于开发依赖，生产依赖只有下述一个低危 Vue 公告。相较 2026-09-25 的 50 个公告基线，工具链修复后当前总数减少 40；该结果用于跟踪当前分支，不以批量覆盖版本的方式处理。
...
- 将 `normalize-package-data@2.3.2` 和 `@2.5.0` 的 `hosted-git-info` 边分别从 2.0.2 / 2.1.4 定向更新至兼容范围内的 2.8.9，修复 [GHSA-43f8-2h32-f4cj](https://github.com/advisories/GHSA-43f8-2h32-f4cj)。从两个父包分别调用 GitHub 仓库 URL 解析 API；全量 build、345 项测试、typecheck、声明、边界、迁移 lint、格式、生产审计及 1,356 个 registry 签名验证通过。CI run [36248444075](https://github.com/Nyakooo/ajax-proxy/actions/runs/36248444075) 的 Stable 和最低版本 Chrome / Edge smoke 全部通过。
...
- `pnpm audit signatures --registry=https://registry.npmjs.org --json` 验证 1,356 个已安装包，invalid / missing 均为 0。
