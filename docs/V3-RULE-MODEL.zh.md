...
- URL matcher 的 `method` 是可选字符串，`type` 可选 `normal` 或 `regex`。重定向 payload 必须含非空目标 `url`；响应替换可选 `status`（200–599 整数）、字符串 header map、JSON `body` 和字符串 `code`。未知 action / payload 字段拒绝。
