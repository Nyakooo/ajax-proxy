...
记录日期：2026-09-25。执行 `pnpm test:coverage`：6 个测试文件、32 个用例通过；workspace 全部 TypeScript 源码的当前总体覆盖率为：
