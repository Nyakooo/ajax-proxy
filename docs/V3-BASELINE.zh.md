...
- V3 基础 schema 和 JSON 解析器位于独立 `@proxy/v3-domain`：当前版本 5 完整备份 envelope 仍读取版本 3 / 4 的旧规则配置，旧备份规范化为无站点停用项的版本 5；严格校验唯一规则 / 标签 ID 和规范 origin，提供路径化错误和 V2 不兼容提示。Vue 3 staging 面板提供备份导出、导入校验和确认恢复；含函数代码的规则会计数提示并在恢复后保持停用。
