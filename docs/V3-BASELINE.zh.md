...
- V2 面板主流程包含全局开关、模式切换、语言选择、JSON 备份 / 恢复，以及拦截和重定向规则列表编辑；拦截规则支持标签管理。
...
- 维护者已确认 V3 不兼容 V2 配置 / 规则 / 备份，不提供自动迁移或转换工具。V3 导入器会识别 V2 格式并明确提示不兼容；V3 备份的使用方式见 `docs/V3-BACKUP-RESTORE.zh.md`。
- V3 规则模型定义完整备份快照语义：必需规则字段中的空数组清空现有列表，缺失 / `null` / 错类型拒绝；校验失败不覆盖当前配置，空规则备份可以导出。domain schema、解析器及面板导入导出流程已实现并有测试覆盖。
- V3 基础 schema 和 JSON 解析器位于独立 `@proxy/v3-domain`：当前版本 6 完整备份 envelope 读取版本 3 / 4 / 5 的旧 V3 配置并规范化为版本 6；版本 6 增加重定向 URL 排除项，严格校验唯一规则 / 标签 ID、规范 origin 及排除项数量 / 长度 / 总字节数，并提供路径化错误和 V2 不兼容提示。Vue 3 staging 面板提供备份导出、导入校验和确认恢复；含函数代码的规则会计数提示并在恢复后保持停用。
- V3 规则支持可选多标签关联；Vue 3 staging 面板可以管理标签、在拦截 / 重定向规则中关联标签、按标签筛选和搜索标签名。删除标签会确认并清理规则引用。
- V2 自定义函数通过 `window.eval()` 在页面主世界执行；当前异步 5 秒计时不能打断同步死循环，面板尚无明确风险提示。V3 已实现隔离执行、函数编辑和导入代码确认；具体边界见 `docs/V3-USER-FUNCTIONS.zh.md`。
