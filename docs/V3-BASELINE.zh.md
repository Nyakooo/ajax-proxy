...
- V3 基础 schema 和 JSON 解析器位于独立 `@proxy/v3-domain`：当前版本 4 完整备份 envelope 仍读取版本 3 的旧 matcher 规则，严格校验唯一规则 / 标签 ID，提供路径化错误和 V2 不兼容提示。Vue 3 staging 面板提供备份导出、导入校验和确认恢复；含函数代码的规则会计数提示并在恢复后保持停用。
