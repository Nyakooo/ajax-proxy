...
| 线索                                                      | 风险                                                                         |
| --------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Storage 缓存在初始化时填充，未见 `storage.onChanged` 订阅 | 面板、多个标签页及 service worker 的设置可能短暂不一致；写入失败没有统一处理 |
...
- 导入空规则列表是清空还是无操作。V2 当前空列表导入不会覆盖旧数据，旧格式转换的字段校验较浅；V3 用新 schema 并明确提示不兼容，不继承 V2 隐式行为。
