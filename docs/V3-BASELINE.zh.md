...
- V3 基础 schema 和 JSON 解析器已落在独立 `@proxy/v3-domain`：版本 3 完整备份 envelope、严格字段校验、唯一规则 / 标签 ID、路径化错误和 V2 不兼容提示；高级 matcher / action 能力与面板导入导出 UI 尚未实现。
- V2 自定义函数通过 `window.eval()` 在页面主世界执行；当前异步 5 秒计时不能打断同步死循环，面板尚无明确风险提示。V3 的隔离执行与导入代码确认要求见 `docs/V3-USER-FUNCTIONS.zh.md`，尚未实现。
