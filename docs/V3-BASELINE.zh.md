...
- V3 基础 schema 和 JSON 解析器位于独立 `@proxy/v3-domain`：版本 3 完整备份 envelope、严格字段校验、唯一规则 / 标签 ID、路径化错误和 V2 不兼容提示。Vue 3 staging 面板提供备份导出、导入校验和确认恢复；含函数代码的规则会计数提示并在恢复后保持停用。
