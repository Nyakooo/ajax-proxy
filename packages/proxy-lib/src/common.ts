...
export function maybeMatching(
  ...
  let matched = false
  if (url.length > MAX_MATCH_INPUT_LENGTH) return matched
  switch (type) {
    // 普通匹配规则
    case 'normal':
    ...
    // 正则匹配规则
    case 'regex':
      matched = getRegex(match)?.test(url) ?? false
      break
  }
  return matched
}
...
export function finalRedirectUrl(
  ...
  let finalUrl = url
  if (url.length > MAX_MATCH_INPUT_LENGTH) return finalUrl
  switch (type) {
    // 普通替换
    case 'normal':
    ...
    // 正则替换
    case 'regex':
      const regex = getRegex(match)
      if (regex) finalUrl = regex.matcher(url).replaceFirst(redirect_url)
      break
  }
  return finalUrl
}
